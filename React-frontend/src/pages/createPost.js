...
const CreatePost = ({ currentUser }) => {
	...
	const createPost = async (e) => {
		...
		const imageName = `${timestamp}_${post.title}`; // Include timestamp in the filename
		const storageRef = ref(storage, `post_images/${imageName}`);

		try {
			await uploadString(storageRef, post.imagePath, "data_url");
		} catch (error) {
		...
		try {
			const res = await axiosInstance.post("/posts", {
				...
				imagePath: imageUrl, // Send the data URL
			...
			axiosErrors.forEach((error) =>
				localErrors.push(` - ${error.title || error.description || error.image || error.video || error.url}`)
			);
	...
	useEffect(() => {
		...
		let labTestTags = [];
		originalTestsWithCapitalisedTags.forEach((test) => labTestTags.push(...test.type));
	...
	return (
		<div>
					...
					<div className="py-2 px-4 pt-3 col-md-8 col-lg-5">
						...
						>
							{post.imagePath === "" ? (
								...
								<img style={{ height: 310, width: 482 }} src={post.imagePath} alt="" />
							)}
						</label>
						<input
							onChange={(e) => {
			...
			frs CFA
						...
						<button onClick={createPost} className=" btn btn-info btn-lg  w-100">
							{!loading ? (
