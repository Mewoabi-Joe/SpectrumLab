...
export const createNewLabTest = async (req: Request, res: Response) => {
  try {
    ...
    if (errors.length === 0) {
      ...
      newTest.image.data = fs.readFileSync(
        "uploads/" + (req as unknown as MulterRequest).file.filename
      )
      newTest.image.size = (req as unknown as MulterRequest).file.size
      newTest.image.contentType = (
...
export const updateLabTest = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!(req as unknown as MulterRequest).file) {
      ...
      if (errors.length === 0) {
        updatedTest = await Test.findByIdAndUpdate(
          ...
          { ...rest, tags: JSON.parse(req.body.tags) }
        )
    ...
    image.size = (req as unknown as MulterRequest).file.size
    image.contentType = (req as unknown as MulterRequest).file.mimetype

    if (errors.length === 0) {
      updatedTest = await Test.findByIdAndUpdate(
        { _id: req.params.testId },
        { ...req.body, image, tags: JSON.parse(req.body.tags) }
      )
      const test = await Test.findOne({ _id: req.params.testId })
...
export const getTestImage = async (req: Request, res: Response) => {
  try {
    const test: any = await Test.findOne({ _id: req.params.testId })
